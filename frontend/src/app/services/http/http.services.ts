...
export class HttpService {
  ...
  get<T>(url, newHeaders: HttpHeaders = null): Observable<DefaultResponse<T>> {
    return this.request<T>('GET', `${url}`, newHeaders);
  }

  delete<T>(url, id: number): Observable<DefaultResponse<T>> {
    return this.request<T>('DELETE', `${url}`, { id });
  }
  delete2<T>(url): Observable<DefaultResponse<T>> {
  ...
  private request<T>(
   ...
   // newHeaders: HttpHeaders = null
    ...
    let headers: HttpHeaders = this.getDefaultHeader(useFormData); 
